...
enum PatchProjectLibrary {
    ...
    static func installBundledPackagesIfNeeded(
        ...
        let extensions = ["3105", "ZeroM$", "cryptroic"]
        var bundledURLs: [URL] = []
        var seen = Set<String>()
    ...
    static func save(
        ...
        if let existingURL {
            ...
            var candidate = root.appendingPathComponent(baseName).appendingPathExtension("ZeroM$")
            var suffix = 2
            while fileManager.fileExists(atPath: candidate.path) {
                candidate = root.appendingPathComponent("\(baseName)-\(suffix)").appendingPathExtension("ZeroM$")
                suffix += 1
            }
